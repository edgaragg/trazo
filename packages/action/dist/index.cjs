...
function links(definition) {
  ...
  return value.filter((v) => typeof v === "string").map((link) => link.split(":")[0] ?? link);
}
...
function extractModel(files, extractors = defaultExtractors, config = {}) {
  ...
  return {
    ...
    edges: [...edges.values()].filter((edge) => nodes.has(edge.from) && nodes.has(edge.to)).sort(byEdge)
  };
}
var edgeKey2 = (edge) => `${edge.from}\0${edge.to}`;
function diffModels(before, after) {
  const beforeNodes = new Map(before.nodes.map((node) => [node.id, node]));
  const afterNodes = new Map(after.nodes.map((node) => [node.id, node]));
  const beforeEdges = new Map(before.edges.map((edge) => [edgeKey2(edge), edge]));
  const afterEdges = new Map(after.edges.map((edge) => [edgeKey2(edge), edge]));
  const changedNodes = [];
  ...
  return {
    ...
    changedNodes,
    addedEdges: after.edges.filter((edge) => !beforeEdges.has(edgeKey2(edge))),
    removedEdges: before.edges.filter((edge) => !afterEdges.has(edgeKey2(edge)))
  };
}
...
function collectAtRef(root, ref) {
  const git = (...args) => (0, import_child_process.execFileSync)("git", args, {
    cwd: root,
    encoding: "utf8",
  ...
  return listing.split("\n").filter((path) => path !== "" && isSupported(path)).map((path) => ({ path, content: git("show", `${ref}:./${path}`) }));
}

// src/action.ts
async function github(apiUrl, token, method, path, body) {
...
async function runAction(env = process.env, log = console.log) {
  ...
  const pullRequest = JSON.parse((0, import_node_fs.readFileSync)(eventPath, "utf8")).pull_request;
  if (!pullRequest) {
  ...
  const comments = await github(apiUrl, token, "GET", `${commentsPath}?per_page=100`);
